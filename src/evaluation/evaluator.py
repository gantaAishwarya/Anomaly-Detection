...
class Evaluator:
    ...
    def evaluate(self):
        for ds in progressbar.progressbar(self.datasets):
            #dse = UCR_UEA_datasets()
            #(X_train, y_train, X_test, y_test) = dse.load_dataset('ECG5000')
            ...
            for det in progressbar.progressbar(self.detectors):
                ...
                try:
                    ...
                    self.plot_prototypes_in_input_space(det, ds, store=True)
                    self.plot_input_sequences_closer_to_prototypes_input_space(det,ds,store=True)
                    try:
    ...
    def plot_latents_and_prototypes(self, det, ds, df, store=True):
        ...
        tnse = TSNE(n_components=2, verbose=0, perplexity=p, n_iter=1000)
        ...
        s = lambda x: 25 if x == 0 else 150
        ss = [s(i) for i in df1.indicator]
        scatter = plt.scatter(tnse_results[:, 0], tnse_results[:, 1],s=ss, marker='.', c=df1.indicator, cmap='bwr_r',label =df1.indicator)
        ...
        for k in range(n_proto):
            ...
            for col in X_train.columns:
                ...
                if (st_ind - (seq_len * 2)) < 0:
                    ...
                    plt.plot(X_train[col].iloc[((st_ind) - (seq_len * 2)):(X_train.shape[0])],
                             label='Original input sequence')
                else:
    ...
    def plot_input_sequences_closer_to_prototypes_input_space(self, det, ds, store=True):
        ...
        for k in range(n_proto):
            ...
            for i in range(0, 15):
                for col in X_train.columns:
                    classes = ['Prototype in input space',
                               'Similar Input sequences(' + str(15) + ')']
                    ...
                    plt.ylabel(f'Proto-{k}')
                    pop_a = mpatches.Patch(color='blue', label='Prototype in input space')
                    pop_b = mpatches.Patch(color='deepskyblue', label='Similar Input sequences(' + str(15) + ')')
